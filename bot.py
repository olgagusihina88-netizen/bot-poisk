...
async def cmd_start(message: types.Message, state: FSMContext):
    await state.clear()
    await message.reply("Привет! Напиши, что ищешь (например, 'цветы').")
...
async def handle_any_message(message: types.Message, state: FSMContext):
    ...
    if current_state is None:
        ...
        if not query:
            await message.reply("Пожалуйста, напишите, что ищете.")
            return
...
async def process_district(message: types.Message, state: FSMContext):
    ...
    items = load_data()
    if not items:
        await message.reply("Ошибка загрузки базы данных.")
        await state.clear()
        return
